...
import { createAnimeWithEpisodes, fetchAniListData, fetchMALEpisodes } from '../services/mockBackend';
...
import { Plus, Trash2, ArrowLeft, Sparkles, Globe, CheckCircle2, AlertCircle, Loader2 } from 'lucide-react';
...
const AddAnimePage = () => {
  ...
  const handleFetchEpisodes = async () => {
    ...
    setMsg(null);
    try {
      const eps = await fetchMALEpisodes(anilistId);
      if (eps.length === 0) {
        setMsg({ type: 'error', text: 'MAL\'dan bölüm verisi bulunamadı. Manuel ekleyebilirsin.' });
        return;
      }
      setEpisodeRows(eps.map(ep => {
        const rowId = `ep-${ep.number}-${Date.now()}`;
        return { id: rowId, number: String(ep.number), title: ep.title, thumbnail: ep.thumbnail, fansubs: [newFansubRow(rowId)] };
      }));
      setMsg({ type: 'success', text: `${eps.length} bölüm çekildi ve eklendi! Fembed linklerini doldurman yeterli.` });
    } catch {
  ...
  const addRow = () => setEpisodeRows(prev => [...prev, newEpisodeRow(String(prev.length + 1))]);
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-[#18181b] border border-gray-800 rounded-2xl p-4 sm:p-6 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-gray-800 pb-3 gap-3">
            <h2 className="text-base font-bold text-white flex items-center gap-2">
              <Plus size={16} className="text-amber-500" /> Bölümler ({episodeRows.length})
            </h2>
            <div className="flex items-center gap-2 flex-wrap">
              {anilistId && (
                ...
                >
                  ...
                  {epFetchLoading ? 'Çekiliyor...' : 'MAL\'dan Bölümleri Çek'}
                </Button>
              )}
              <p className="text-xs text-gray-500 hidden sm:block">Opsiyonel — sonradan da ekleyebilirsin</p>
            </div>
          </div>
          ...
          <div className="space-y-3 max-h-[50vh] overflow-y-auto pr-1">
            {episodeRows.length === 0 && (
              <p className="text-gray-600 text-sm text-center py-4">Henüz bölüm eklenmedi.</p>
            )}
            {episodeRows.map((row, idx) => (
              <div key={row.id} className="bg-gray-900 border border-gray-800 rounded-xl p-3 sm:p-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={() => removeRow(row.id)} className="text-red-500 hover:text-red-400 p-1.5"><Trash2 size={15}/></button>
                </div>
                  ...
                  {row.fansubs.map((fb, fi) => (
                    <div key={fb.id} className="bg-gray-800/60 border border-gray-700/60 rounded-lg p-2.5 space-y-2">
                      <div className="flex items-center gap-2">
                        <input type="text" placeholder="Fansub adı (ör: TurkAnime)" value={fb.name} onChange={e => updateFansubName(row.id, fb.id, e.target.value)} className="flex-1 bg-gray-800 border border-gray-600 rounded px-2 py-1.5 text-white text-xs font-bold focus:border-blue-500 focus:outline-none" />
                        {row.fansubs.length > 1 && <button type="button" onClick={() => removeFansub(row.id, fb.id)} className="text-red-500 p-1"><Plus size={13} className="rotate-45"/></button>}
                      </div>
                      {fb.sources.map((src, si) => (
                        <div key={src.id} className="flex gap-1.5 items-center">
                          <input type="text" list="pp-presets" placeholder="Sibnet..." value={src.name} onChange={e => updateSource(row.id, fb.id, src.id, 'name', e.target.value)} className="w-20 flex-shrink-0 bg-gray-800 border border-gray-600 rounded px-2 py-1.5 text-white text-xs focus:border-amber-500 focus:outline-none" />
                          ...
                          {fb.sources.length > 1 && <button type="button" onClick={() => removeSource(row.id, fb.id, src.id)} className="text-red-500 p-1 flex-shrink-0"><Trash2 size={12}/></button>}
                        </div>
                      ))}
          ...
          <button type="button" onClick={addRow} className="w-full flex items-center justify-center gap-2 py-3 border-2 border-dashed border-gray-700 rounded-xl text-gray-400 hover:border-amber-500 hover:text-amber-500 transition-colors text-sm font-medium">
            <Plus size={18} /> Bölüm Satırı Ekle
          </button>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <Button type="submit" isLoading={loading} className="flex-2 flex-grow-[2]">
            {isModerator ? 'Onaya Gönder' : `Yayınla${episodeRows.length > 0 ? ` (${episodeRows.length} Bölüm)` : ''}`}
          </Button>
        </div>
      </form>
    </div>
